...
public class FacilityRepoImpl implements FacilityServiceRepo {
    ...
    public static void main(String[] args) {
        System.out.println(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.NAME_SERVICE_VILLA));
    }

    @Override
    public void display() {
        LinkedHashMap<Facility, Integer> facilityServiceList = FacilityRepoImpl.readFacility();
    ...
    public void add() {
        int choose;
        LinkedHashMap<Facility, Integer> list = FacilityRepoImpl.readFacility();
        do {
            ...
            if (choose == 1) {
                ...
                String typeService = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE);
                System.out.println("input id service (SVVL-XXXX, X is number): ");
                String nameService = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.NAME_SERVICE_VILLA);
                System.out.println("input usable area: ");
                double usableArea = Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA));
                System.out.println("input rental cost: ");
                int rentalCost = Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.COST_FLOOR));
                System.out.println("input maximum number of people: ");
                int maxPeople = Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.MAX_PEOPLE));
                System.out.println("input rental type: ");
                String rentalType = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE);
                System.out.println("input standar room: ");
                String standarRoom = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE);
                System.out.println("input pool area: ");
                double areaPool = Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA));
                System.out.println("input floor: ");
                int floor = Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.COST_FLOOR));
            ...
            if (choose == 2) {
                ...
                String typeService = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE);
                System.out.println("input id service (SVVL-XXXX, X is number): ");
                String nameService = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.NAME_SERVICE_HOUSE);
                System.out.println("input usable area: ");
                double usableArea = Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA));
                System.out.println("input rental cost: ");
                int rentalCost = Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.COST_FLOOR));
            ...
            if (choose == 3) {
                ...
                String serviceFree = FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE);
                list.put(new Room(typeService, nameService, usableArea, rentalCost, maxPeople, rentalType,
                        serviceFree), 0);
            }
            FacilityRepoImpl.writerFacility(list);
        } while (choose != 0);
    }
                ...
                if (key instanceof Villa) {
                    ...
                    do {
                        ...
                        if (chooseVilla == 1) {
                            ...
                            key.setServiceID(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.NAME_SERVICE_VILLA));
                        }
                        if (chooseVilla == 2) {
                            System.out.println("input usable area: ");
                            key.setAreaUsable(Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA)));
                        }
                        if (chooseVilla == 3) {
                            System.out.println("input rental cost: ");
                            key.setRentalCost(Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.COST_FLOOR)));
                        }
                        if (chooseVilla == 4) {
                            System.out.println("input maximum number of people: ");
                            key.setMaxPeople(Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.MAX_PEOPLE)));
                        }
                        if (chooseVilla == 5) {
                            System.out.println("input rental type: ");
                            key.setRentalType(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE));
                        }
                        if (chooseVilla == 6) {
                            System.out.println("input standar room: ");
                            ((Villa) key).setStandardRoom(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE));
                        }
                        if (chooseVilla == 7) {
                            System.out.println("input pool area: ");
                            ((Villa) key).setAreaPool(Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA)));
                        }
                        if (chooseVilla == 8) {
                            System.out.println("input floor: ");
                            ((Villa) key).setFloor(Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.COST_FLOOR)));
                        }
                        FacilityRepoImpl.writerFacility(list);
                    } while (chooseVilla != 0);
                }
                if (key instanceof House) {
                    ...
                    do {
                        ...
                        if (chooseHouse == 1) {
                            ...
                            key.setServiceID(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.NAME_SERVICE_HOUSE));
                        }
                        if (chooseHouse == 2) {
                            System.out.println("input usable area: ");
                            key.setAreaUsable(Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA)));
                        ...
                        if (chooseHouse == 7) {
                            System.out.println("input floor: ");
                            ((House) key).setFloor(Integer.parseInt(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.COST_FLOOR)));
                        }
                        FacilityRepoImpl.writerFacility(list);
                    } while (chooseHouse != 0);
                }
                if (key instanceof Room) {
                    ...
                    do {
                        ...
                        if (chooseRoom == 1) {
                            ...
                            key.setServiceID(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.NAME_SERVICE_ROOM));
                        }
                        if (chooseRoom == 2) {
                            System.out.println("input usable area: ");
                            key.setAreaUsable(Double.parseDouble(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.AREA)));
                        ...
                        if (chooseRoom == 6) {
                            System.out.println("input free service: ");
                            ((Room) key).setServiceFree(FacilityRepoImpl.validateInput(scanner.nextLine(), Validate.SERVICE_TYPE));
                        }
                        System.out.println(((Room) key).showRoom());
                        FacilityRepoImpl.writerFacility(list);
                    } while (chooseRoom != 0);
